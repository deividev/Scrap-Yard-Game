...
@Component({
  ...
  template: `
    <div class="factory-view">
      ...
      <div class="zone-nav">
        @for (tier of tieredMachines(); track tier.labelKey; let i = $index) {
          <button
            ...
            [class.zone-btn--locked]="isZoneLocked(i)"
            [attr.data-zone]="i"
            [disabled]="isZoneLocked(i)"
            (click)="!isZoneLocked(i) && selectedZone.set(i)"
          >
      ...
      <!-- ── MACHINES PANEL ── -->
      @for (tier of tieredMachines(); track tier.labelKey; let i = $index) {
        @if (selectedZone() === i && !isZoneLocked(i)) {
...
export class MachineListComponent {
  ...
  isZoneLocked(index: number): boolean {
    return this.demoService.isDemo && index > 0;
  }
}
